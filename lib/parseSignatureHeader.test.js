...
tap.test('Signature header is extracted as a dictionary.', async ({ same }) => {
  const tests = [
    {
...
tap.test('Errors are thrown when input is unparseable', async ({ throws }) => {
  const tests = [
    {
      description: 'No signature header returns an error',
      input: { headers: {} },
      expected: new Error('Request not signed.')
    }
  ]
